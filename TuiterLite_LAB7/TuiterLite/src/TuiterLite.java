...
public class TuiterLite<T> {
     ...
     */
    public Tuite tuitarAlgo(Usuario usuario, String texto) throws TuiteExcedenteException, UsuarioDesconhecidoException {

        if (texto.length() > TAMANHO_MAXIMO_TUITES) {
            throw  new TuiteExcedenteException();
        }

        Usuario usuarioExistente = this.usuarioByEmail.get(usuario.getEmail());
        // verifica se o usuário é conhecido
        if (usuarioExistente == null) {
            throw new UsuarioDesconhecidoException();  // ToDo lançar uma exceção
        }
